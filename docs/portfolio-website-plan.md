...
**Logo Drop Cluster (between Hero CTAs and Zone 2 title)**

A signature moment that translates "the stack I work with" into something physical and playful, while remaining tasteful.

- **What it is:** the `image` field on every `taxonomy.json` entry (Simple Icons SVG URLs for languages / libraries / technologies) is collected into a single set of unique logos. On page load, those logos **drop from the top of the viewport** under gravity and **bounce into a loose, messy cluster** in a dedicated band that sits **between the Hero CTA pair and Zone 2's "What are you looking for?" search title.**
...
- **Visual target:** soft physics, not perfect grid. Logos overlap slightly, sit at slightly varied rotations (-12° to +12°), with subtle drop shadows. The cluster reads as a single object, not 30 floating icons.
- **Motion spec:**
  - Each logo drops from `y: -120vh + random offset`, staggered by ~20–40ms each
  - Settles at its target position via spring (`stiffness: 80, damping: 12, mass: 0.8` as a starting point — tune in implementation)
  - 1–2 secondary bounces, never pixel-perfect — slight horizontal jitter and rotation overshoot read as "real"
  - Total animation budget: under ~1.6s from first drop to fully settled
  - Hover on any logo: gentle lift + tooltip with the tag display name; click optionally adds that slug as an active filter chip (extension, not required v1)
- **Layout:**
  - Dedicated band, full-width, ~200–260px tall on desktop, ~140–180px on mobile
  - Logos absolutely positioned inside the band; cluster centered horizontally
  - Logo size: ~40–48px on desktop, ~28–32px on mobile; SVG, not raster
- **Accessibility:**
  - Wrap in a region with `aria-label="Technologies I work with"` and a visually hidden `<ul>` of tag names for screen readers
  - Respect `prefers-reduced-motion`: no drop, no bounce — logos fade in at their final positions instead
...
- **Implementation home:** new component `src/components/hero/LogoDropCluster.tsx`, consumed by `Hero`. Physics via Framer Motion springs (no extra library). Renders nothing until after hydration to avoid SSR mismatch on randomized positions — a deterministic seed (e.g. hash of slug) keeps positions stable across renders.
