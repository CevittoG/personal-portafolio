...
The **scaffold + Tag Pill + Navbar/Footer + Experience Card + SearchBar (Build Order steps 1–7) is in place**: Next.js 15.4.11 + TS + Tailwind v4 app, Docker dev/preview pipeline, color tokens, typed empty JSON data, a SOLID-aligned `src/lib` layer (taxonomy, experience, filters, related, stats, search — interfaces + impls + `format.ts` helpers for both domains + `top-tags.ts`), the `cn()` utility, the `TagPill` primitive with all four states (`src/components/tags/`), a global shell (`Navbar` + `Footer` under `src/components/layout/`, nav items and socials driven by config), the `ExperienceCard` component, and the `SearchBar` accessible combobox (`src/components/search/`) wired with a pluggable `SearchStrategy` (substring default). The placeholder `/` drives `ExperienceCard.filterTags` live from the SearchBar through a removable-pill chip row (preview of step 8). The next pass starts at **Build Order step 8: Active Filter Chips + URL sync**.
