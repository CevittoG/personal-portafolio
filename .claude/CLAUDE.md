...
The **scaffold + Tag Pill + Navbar/Footer + Experience Card + SearchBar + Active Filter Chips (Build Order steps 1–8) is in place**: Next.js 15.4.11 + TS + Tailwind v4 app, Docker dev/preview pipeline, color tokens, typed empty JSON data, a SOLID-aligned `src/lib` layer (taxonomy, experience, filters, related, stats, search — interfaces + impls + `format.ts`/`labels.ts` helpers + `top-tags.ts` + `useFilterTags()` URL-sync hook), the `cn()` utility, the `TagPill` primitive with all four states plus an optional `sublabel` slot (`src/components/tags/`), a global shell (`Navbar` + `Footer` under `src/components/layout/`, nav items and socials driven by config), the `ExperienceCard` component, the `SearchBar` accessible combobox (`src/components/search/`) wired with a pluggable `SearchStrategy` (substring default), and `ActiveFilterChips` (`src/components/filters/`) rendering URL-synced removable chips. The placeholder `/` drives `ExperienceCard.filterTags` live from the SearchBar through the chip row, and the URL `?tags=…` reflects picks across reload + back/forward. The next pass starts at **Build Order step 9: Stats Bar (computed from filtered entries)**.
